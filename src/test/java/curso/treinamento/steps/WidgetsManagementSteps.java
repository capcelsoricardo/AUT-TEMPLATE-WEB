...
public class WidgetsManagementSteps {
	...
	public void que_eu_esteja_na_tela_Widgets_Management() {
		...
		Assert.assertTrue(widgetsManagementPage.validar_pagina(), "Página Widgets Management NÃO apresentada.");
	}
	...
	public void widget_cadastrado_com_sucesso() {
		Assert.assertTrue(widgetsManagementPage.validar_nome_widget_na_tabela(Helper.dataTable.get(0).get("Widget Name")), "Widget não cadastrado com sucesso.");
	}
	...
	public void altero_informações_do_widgets(DataTable dataTable) {
		...
		Assert.assertTrue(widgetsManagementPage.validar_nome_widget_na_tabela(Helper.dataTable.get(0).get("Widget Name Antigo")), "Widget Name " + Helper.dataTable.get(0).get("Widget Name Antigo") + " não encontrado para alteração.");
	...
	public void informacoes_do_widgets_alterada_com_sucesso() {
		Assert.assertTrue(widgetsManagementPage.validar_nome_widget_na_tabela(Helper.dataTable.get(0).get("Widget Name Novo")), "Widget Name " + Helper.dataTable.get(0).get("Widget Name Novo") + " não encontrado para alteração.");
	}
	...
	public void widgets_excluido_com_sucesso(String nome) {
		Assert.assertFalse(widgetsManagementPage.validar_nome_widget_na_tabela(nome), "Administrador " + nome + " não excluido!");
	}
}
